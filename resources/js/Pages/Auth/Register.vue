...
const fetchAddress = async () => {
  if (!form.zip_code) {
    form.errors.zip_code = 'O CEP é obrigatório.';
    return;
  }
  ...
  try {
    ...
    if (data.erro) {
      form.errors.zip_code = 'CEP não encontrado.';
    } else {
      form.state = data.uf;
    ...
    form.errors.zip_code = 'Erro ao buscar informações do CEP.';
  }
};
...
const validateStep = (currentStep) => {
  ...
  if (currentStep === 1) {
    if (!form.first_name)
      form.errors.first_name = 'O primeiro nome é obrigatório.';
...
<template>
  ...
  <div class="min-h-screen bg-gray-100 flex items-center justify-center">
    <div class="w-full max-w-md bg-white p-8 shadow-md rounded-lg py-8">
      <h1 class="text-2xl font-bold text-center text-gray-700 py-3">
        Realizar Cadastro
      </h1>
      ...
      <div class="flex items-center justify-between mb-8">
        <template v-for="n in totalSteps" :key="n">
          <div class="flex items-center space-x-2">
            ...
            <div
              v-if="n < totalSteps"
              class="h-1 w-16 bg-gray-300"
              :class="isCompletedStep(n) && 'bg-green-500'"
            ></div>
          </div>
        </template>
      </div>
