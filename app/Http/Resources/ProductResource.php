...
class ProductResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            'id' => $this->id,
            'name' => $this->name ?? 'Não informado',  // Caso o nome seja nulo, retorna 'Não informado'
            ...
            'empresa_id' => $this->empresa_id ?? 'Não informado',  // Substitui null por 'Não informado'
            'is_managed' => $this->is_managed ?? false,  // Caso o gerenciamento seja nulo, define como false
            'image_path' => $this->image_path ?? 'imagem-padrao.jpg',  // Se a imagem for nula, usa uma imagem padrão
            'ncm_code' => $this->ncm_code ?? 'Não informado',  // Substitui null por 'Não informado'
            'supplier_id' => $this->supplier_id ?? 'Não informado',  // Substitui null por 'Não informado'
            'expiration_date' => $this->expiration_date ? Carbon::parse($this->expiration_date)->format('d/m/Y') : 'Sem data de expiração',  // Formata a data ou retorna uma string padrão
